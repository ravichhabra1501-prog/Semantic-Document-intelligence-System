...
export default function Settings() {
  ...
  const handleCompactViewChange = (val: boolean) => {
    ...
    toast({
      ...
      description: `Compact view ${val ? "enabled" : "disabled"}.`,
    });
  };
  ...
  const handleNotificationsChange = (val: boolean) => {
    ...
    toast({
      title: "Preference saved",
      description: `Upload notifications ${val ? "enabled" : "disabled"}.`,
    });
  };
  ...
  const SUPPORTED_TYPES = [
    {
      ext: ".PDF",
      label: "PDF Documents",
      color: "bg-red-50 text-red-600 dark:bg-red-500/10",
    },
    {
      ext: ".DOCX",
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-10 glass-panel border-b border-border/50 px-6 lg:px-10 py-6 shadow-elevation animate-fade-in-down">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Customize your workspace preferences and manage your data.
          </p>
        </div>
      </header>
      ...
      <main className="p-6 lg:p-10 max-w-3xl mx-auto space-y-6">
        ...
        <SectionCard icon={Palette} title="Appearance" iconColor="text-primary">
          ...
          <SettingRow
            label="Compact View"
            description="Show smaller document cards on the dashboard"
          >
            <Switch
              checked={compactView}
        ...
        >
          <SettingRow
            ...
            description="Show a toast notification when documents finish processing"
          >
            <Switch
        ...
        >
          <SettingRow
            label="AI Model"
            description="Model used for document analysis, summarization, and entity extraction"
          >
          ...
          <SettingRow
            label="Text extraction limit"
            description="Maximum characters sent to AI per document"
          >
            <Badge variant="secondary" className="font-mono text-xs">
              10,000 chars
            </Badge>
          ...
          <SettingRow
            ...
            description="Formats accepted for upload and processing"
          >
            <div className="flex flex-wrap gap-1.5 max-w-[200px] justify-end">
              {SUPPORTED_TYPES.map((t) => (
                <span
                  key={t.ext}
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${t.color}`}
                >
                  {t.ext}
                </span>
              ))}
        ...
        <SectionCard icon={Info} title="About" iconColor="text-emerald-500">
          ...
          <SettingRow
            label="Stack"
            description="Powered by modern web technologies"
          >
            <div className="flex flex-wrap gap-1 max-w-[200px] justify-end">
              {["React", "TypeScript", "Express", "PostgreSQL", "OpenAI"].map(
                (t) => (
                  <Badge
                    key={t}
                    variant="outline"
          ...
          <SettingRow
            ...
            description="Connected to OpenAI for document analysis"
          >
